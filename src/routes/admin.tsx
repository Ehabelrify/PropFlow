...
import { Building2, Users, Activity, Ban, Plus, MoreHorizontal, ShieldCheck, Server } from "lucide-react";
...
export const Route = createFileRoute("/admin")({
  head: () => ({ meta: [
    { title: "Platform Admin — PropFlow CRM" },
    { name: "description", content: "Super admin console: manage tenants, plans, and platform health." },
  ] }),
  component: AdminPage,
});
      ...
      <div className="flex min-h-[60vh] items-center justify-center px-6">
        <Card className="max-w-md p-8 text-center shadow-card">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            The platform admin console is only available to Super Admins. Switch role from the topbar to preview.
          </p>
        </Card>
      </div>
    );
    ...
    <div>
      <PageHeader
        ...
        actions={
          <Button size="sm" className="bg-gradient-brand text-primary-foreground">
            <Plus className="mr-1.5 h-4 w-4" /> Provision tenant
          </Button>
        }
      />
      <div className="space-y-6 p-6">
        ...
        <Card className="p-5 shadow-card">
          ...
          <p className="text-xs text-muted-foreground">Service-level snapshot.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {[
              { k: "API uptime (30d)", v: "99.98%" },
              { k: "Avg p95 latency", v: "184 ms" },
              { k: "Background jobs", v: "12 / min" },
            ].map(s => (
