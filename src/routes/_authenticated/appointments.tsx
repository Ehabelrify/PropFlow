      ...
      <div className="space-y-4 p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map(a => {
            ...
            const prop = a.propertyId ? properties.find(p => p.id === a.propertyId) : null;
            const agent = users.find(u => u.id === a.assignedTo);
            const isUpcoming = a.status === "scheduled" && new Date(a.scheduledAt) > new Date();
              ...
              <Card key={a.id} className={`p-5 shadow-card ${isUpcoming ? `ring-1 ring-info/30` : ``}`}>
                ...
                <div className="mt-2 space-y-1.5 text-xs text-muted-foreground">
                  <div className="flex items-center gap-2">
                    ...
                    {format(new Date(a.scheduledAt), "EEEE, MMM d · h:mm a")}
                  </div>
                ...
                {a.status === "scheduled" && (
                  <div className="mt-3 flex gap-2">
                    <Button size="sm" variant="outline" className="text-xs" onClick={() => setAppointmentStatus(a.id, "completed")}>Mark completed</Button>
                    <Button size="sm" variant="outline" className="text-xs text-destructive" onClick={() => setAppointmentStatus(a.id, "cancelled")}>Cancel</Button>
                  </div>
                )}
              </Card>
            );
